...
/** Hero thương hiệu dùng chung cho cả màn gate lẫn màn chào. */
function BrandHero() {
  const t = useT();
  return (
...
/**
 * Màn hình bắt đầu trước khi gọi món.
 * Bàn CHỈ đến từ việc quét QR tại bàn (/?table=N) — không nhập tay, không đổi bàn.
 * Chưa có bàn (chưa quét QR) → màn gate, chặn vào builder.
 */
export default function WelcomeScreen({
  ...
  // Chưa quét QR → không có bàn → gate, không cho gọi món.
  if (tableNo == null) {
    return (
      <main className="fade-in mx-auto flex max-w-md flex-col gap-3.5 px-4 pb-10 pt-5">
        <BrandHero />
        <section className="flex flex-col items-center gap-3 rounded-2xl border border-black/5 bg-white p-6 text-center shadow-soft">
          <div className="flex h-16 w-16 items-center justify-center rounded-full bg-brand-50 text-4xl">
            📷
          </div>
          <h2 className="text-lg font-extrabold tracking-tight text-ink">
            {t("welcome.scanGateTitle")}
          </h2>
          <p className="max-w-xs text-sm text-ink/55">{t("welcome.scanGateBody")}</p>
        </section>
        <p className="text-center text-xs text-ink/40">{t("welcome.thanks")}</p>
      </main>
    );
  }
  ...
  return (
    <main className="fade-in mx-auto flex max-w-md flex-col gap-3.5 px-4 pb-10 pt-5">
      ...
      {/* Lời chào + bàn (chỉ đọc, lấy từ QR) */}
